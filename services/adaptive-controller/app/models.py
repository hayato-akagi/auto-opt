...
class AdaptiveControllerConfig(BaseModel):
    ...
    # EMA weight for bolt_shift estimate update: 1.0 = use latest observation only
...
class ControlRunRequest(BaseModel):
    ...
    algorithm: str = Field(default="adaptive-controller")
    config: AdaptiveControllerConfig = Field(default_factory=AdaptiveControllerConfig)
    target: TargetSpot
...
class ControlStepState(BaseModel):
    ...
    # Current bolt_shift estimate (updated each step by runner)
...
class ControlStepInfo(BaseModel):
    ...
    adaptive_delta_x: float
    adaptive_delta_y: float
    bolt_shift_estimate_x: float
    bolt_shift_estimate_y: float
    clipped_x: bool
    clipped_y: bool
...
class InitialObservation(BaseModel):
    ...
    initial_bolt_shift_x: float
    initial_bolt_shift_y: float
