"""Trial control loop for adaptive-controller.

Maintains bolt_shift_estimate across steps within a single trial.
The estimate is initialized from Step 0's observation, then updated
with an EMA after each executed step.
"""
...
from .logic import compute_step, update_bolt_shift_estimate
from .models import (
...
class LoopState:
    commanded_x: float
    commanded_y: float
    bolt_shift_estimate_x: float = field(default=0.0)
    bolt_shift_estimate_y: float = field(default=0.0)
    perturb_x: float = field(default=0.0)
    perturb_y: float = field(default=0.0)
...
def _observe_bolt_shift(step_result: dict[str, Any]) -> tuple[float, float]:
    """Extract bolt shift from a step result: spot_after_bolt - spot_after_position."""
    sim_pos = step_result.get("sim_after_position", {})
    sim_bolt = step_result.get("sim_after_bolt", {})
    shift_x = float(sim_bolt.get("spot_center_x", 0.0)) - float(sim_pos.get("spot_center_x", 0.0))
    shift_y = float(sim_bolt.get("spot_center_y", 0.0)) - float(sim_pos.get("spot_center_y", 0.0))
...
async def run_control_loop(request: ControlRunRequest, client: RecipeClient) -> ControlRunResponse:
    ...
    config: AdaptiveControllerConfig = request.config
    ...
    # Initialize bolt_shift estimate from Step 0 observation
    initial_bolt_shift_x, initial_bolt_shift_y = _observe_bolt_shift(step0)

    boot_correction_x = (request.target.spot_center_x - pre_x0) / config.spot_to_coll_scale_x
    boot_correction_y = (request.target.spot_center_y - pre_y0) / config.spot_to_coll_scale_y

    state = LoopState(
        commanded_x=request.initial_coll.coll_x + boot_correction_x,
        commanded_y=request.initial_coll.coll_y + boot_correction_y,
        bolt_shift_estimate_x=initial_bolt_shift_x,
        bolt_shift_estimate_y=initial_bolt_shift_y,
    )
    ...
    initial_observation = InitialObservation(
        ...
        initial_bolt_shift_x=initial_bolt_shift_x,
        initial_bolt_shift_y=initial_bolt_shift_y,
    )
    ...
    for _ in range(request.max_steps):
        ...
        # Update bolt_shift estimate from this step's observation
        obs_x, obs_y = _observe_bolt_shift(step_result)
        state.bolt_shift_estimate_x, state.bolt_shift_estimate_y = update_bolt_shift_estimate(
            state.bolt_shift_estimate_x,
            state.bolt_shift_estimate_y,
            obs_x,
            obs_y,
            config.alpha,
        )
        ...
        state.perturb_x = rng.gauss(0.0, config.release_perturbation.std_x)
        state.perturb_y = rng.gauss(0.0, config.release_perturbation.std_y)
