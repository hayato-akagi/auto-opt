"""LSTM controller trial loop.

The LSTM hidden state (h, c) is initialized to None at trial start
and threaded through each step.  This lets the LSTM accumulate environment
context within a trial without any state leaking between trials.
"""
...
from .logic import compute_lstm_step, make_lstm_features
...
class LoopState:
    commanded_x: float
    commanded_y: float
    lstm_hidden_state: tuple | None = field(default=None)
...
async def run_control_loop(
    ...
    # Hidden state starts as None (LSTM uses zeros internally on first call)
    state = LoopState(
    ...
    # last_step is used to build LSTM features for the next step.
    # Step 0's result IS the "previous step" for the first loop iteration.
    last_step: dict[str, Any] = step0
    steps_executed = 0
    ...
    for _ in range(request.max_steps):
        ...
        action, new_hidden = compute_lstm_step(
            config=config,
        ...
        # Persist hidden state for the next step
        state.lstm_hidden_state = new_hidden
        ...
        ai_step_log = {
            ...
            "dnn_residual_x": action.lstm_residual_x,
            "dnn_residual_y": action.lstm_residual_y,
