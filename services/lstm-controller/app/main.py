...
SUPPORTED_ALGORITHM = "lstm-controller"
...
def create_app(
    ...
    app = FastAPI(title="lstm-controller", version="0.1.0", lifespan=lifespan)
    ...
    async def control_run(payload: ControlRunRequest) -> ControlRunResponse:
        ...
        if payload.config.model_path:
            ...
            return await run_control_loop(payload, resolved_client, temp_mm)
        ...
        if payload.config.model_version is None:
            ...
            payload.config.model_type = str(
                status.get("model_type") or payload.config.model_type
            )
        return await run_control_loop(payload, resolved_client, resolved_model_manager)
    ...
    async def health() -> HealthResponse:
        return HealthResponse(
            status="ok", service="lstm-controller", version="0.1.0"
        )
