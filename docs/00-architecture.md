...
```
Streamlit → Recipe: "XY=(0.02, -0.05), トルク=(0.5, 0.5) で実行"
  Recipe → Position: XY位置指定 → coll_x/y_shift 返却
  Recipe → Optics Sim: 位置調整後パラメータで追跡 → 結果A保存
  Recipe → Bolt: トルク指定 → Δx, Δy 返却
...
Streamlit → Controller: POST /control/run
  ...
  ループ開始:
    Controller: PID演算 → 次の coll_x, coll_y, トルクを決定
